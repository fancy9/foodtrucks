...
UccDBQuery::UccDBQuery(string collection, string locationType,
		const string* location, const string* radius, const string* limit, const string* offset, const string* foodItems,
...
void UccDBQuery::processLocation(string locationType, const string* location,
		const string *radius) {
...
void UccDBQuery::generateQueryCommand() {
	...
	if(this->offset > 0) {
		BSONObjBuilder offset_builder;
		offset_builder.append("$skip", this->offset);
		array_builder.append(offset_builder.obj());
	}
...
BSONObj& UccDBQuery::getQueryCommand() {
	return this->command;
}
